...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`<a:SabitGif:714416499233652758> ${member} **Adlı Kişi Sunucuya Katıldı** \n<a:rainbowleft:787811922027348069> **${invite.inviter.tag}** adlı kişinin **${total + bonus}** adet daveti oldu.`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`**${member.user.tag}** Sunucudan Ayrıldı <:ElSallamaGif:714413917627219988> \n ${user.tag} adlı kişinin **${Number(total) + Number(bonus)}** Daveti kaldı <a:Reddetmek:714416254206607410>`)
     }
});
